...
func loggerWith(logger *slog.Logger, protoMsg protoreflect.ProtoMessage) *slog.Logger {
	grpcFields := trace.FieldsFor(protoMsg)
...
func loggerInterceptor(rootLogger *slog.Logger) grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		...
		logger = loggerWith(logger.WithGroup("req"), req.(protoreflect.ProtoMessage))
		logger.Log(ctx, slog.LevelDebug, "received GRPC Request")
		...
		if err != nil {
			...
			logger = loggerWith(logger.WithGroup("res"), res.(protoreflect.ProtoMessage))
			logger.Log(ctx, slog.LevelDebug, "returning GRPC Response")
		}
